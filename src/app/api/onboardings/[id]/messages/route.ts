...
export async function GET(
  ...
  const myLastReadAt = (reads as any[])?.find((r) => r.side === "admin")?.last_read_at ?? null;
  const theirLastReadAt = (reads as any[])?.find((r) => r.side === "client")?.last_read_at ?? null;

  if (markRead) {
    await admin.from("onboarding_chat_reads").upsert(
      { onboarding_id: id, side: "admin", last_read_at: new Date().toISOString() },
