...
// ─── Atlanta 2026 Guide (full recreation of Monday.com guide) ────────────────
...
// ─── Stockholm placeholder guide ─────────────────────────────────────────────

const STOCKHOLM_GUIDE = {
  title: "DreamHack Stockholm 2026 — Exhibitor Guide",
  hero_image: "https://images.unsplash.com/photo-1501621667575-af81f1f0bacc?w=1600&q=80",
  intro: "Welcome to DreamHack Stockholm 2026. This guide will be updated as event details are confirmed. Please check back regularly.",
  sections: [
    { id: id(), type: "heading", title: "Overview" },
    {
      id: id(), type: "info_box",
      title: "Guide in progress",
      content: "The full exhibitor guide for DreamHack Stockholm 2026 is being prepared and will be published here shortly. Key dates, venue details, and logistics will be added as they are confirmed.\n\nFor urgent questions, contact exhibitors@dreamhack.com.",
      color: "blue"
    },
    { id: id(), type: "heading", title: "Key Contacts" },
    {
      ...
      rows: [
        ...
        ["Event Operations",         "operations@dreamhack.com"],
      ]
    },
    { id: id(), type: "heading", title: "Venue" },
    {
      id: id(), type: "text",
      title: "Venue details coming soon",
      content: "Venue and hall information for DreamHack Stockholm 2026 will be confirmed and published here. Please check back."
    },
  ]
};

// ─── Birmingham placeholder guide ────────────────────────────────────────────

const BIRMINGHAM_GUIDE = {
  title: "DreamHack Birmingham 2026 — Exhibitor Guide",
  hero_image: "https://images.unsplash.com/photo-1569012871812-f38ee64cd54c?w=1600&q=80",
  intro: "Welcome to DreamHack Birmingham 2026. This guide will be updated as event details are confirmed. Please check back regularly.",
  sections: [
    { id: id(), type: "heading", title: "Overview" },
    {
      id: id(), type: "info_box",
      title: "Guide in progress",
      content: "The full exhibitor guide for DreamHack Birmingham 2026 is being prepared and will be published here shortly. Key dates, venue details, and logistics will be added as they are confirmed.\n\nFor urgent questions, contact exhibitors@dreamhack.com.",
      color: "blue"
    },
    { id: id(), type: "heading", title: "Key Contacts" },
    {
    ...
    { id: id(), type: "heading", title: "Venue" },
    {
      id: id(), type: "text",
      title: "Venue details coming soon",
      content: "Venue and hall information for DreamHack Birmingham 2026 will be confirmed and published here. Please check back."
    },
  ]
};
...
const { data: events } = await sb.from("events").select("id, name").eq("org_id", ORG_ID);
console.log("Events found:", events?.map(e => `${e.id.slice(0,8)} ${e.name}`));

const atlantaEvent = (events ?? []).find(e =>
  e.name?.toLowerCase().includes("atlanta") || e.name?.toLowerCase().includes("atl")
);

if (atlantaEvent) {
  const { error } = await sb.from("events").update({ exhibitor_guide: ATLANTA_GUIDE }).eq("id", atlantaEvent.id);
  console.log(`Atlanta guide: ${error ? "ERROR: " + error.message : "OK"} (${atlantaEvent.name})`);
}

// ─── Stockholm ───────────────────────────────────────────────────────────────

const { error: sErr } = await sb.from("events").update({ exhibitor_guide: STOCKHOLM_GUIDE }).eq("id", STOCKHOLM);
console.log(`Stockholm guide: ${sErr ? "ERROR: " + sErr.message : "OK"}`);

// ─── Birmingham ──────────────────────────────────────────────────────────────

const { error: bErr } = await sb.from("events").update({ exhibitor_guide: BIRMINGHAM_GUIDE }).eq("id", BIRMINGHAM);
console.log(`Birmingham guide: ${bErr ? "ERROR: " + bErr.message : "OK"}`);

console.log("\nDone. Guide URLs:");
if (atlantaEvent) console.log(`  Atlanta:    /guide/${atlantaEvent.id}`);
console.log(`  Stockholm:  /guide/${STOCKHOLM}`);
console.log(`  Birmingham: /guide/${BIRMINGHAM}`);
